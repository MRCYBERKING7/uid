...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://www.facebook.com/rafivaim.m.m"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/rafivaim.m.m")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN🥰MR.RAFI")
	subprocess.check_output(["am", "start", "https://wa.me/+01401607898"])

def irfan():
	
	
	runtxt("\n\033[0;91m😇This tool is PRO ")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://pastebin.com/raw/CmP8Q7wZ').text
			if basesplit in plr:
				key = basesplit
...
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : ANAMUL SHKAE   
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mFB GROUP : NAI
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB   : MRCYBAERERROR
\033[1;91m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : PEID
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sRAFI🥰PROO CLONER🇧🇩"%(G,R,G,B,G))
		...
		print("%s [%s8%s]%s CRACK RANDOM FB ID 2003-4 %s(VIP) V3"%(R,G,R,Y,G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: rafi): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MR] : %s/%s -> \033[0;92m [ MR-OK:%s ]- \033[0;91m[MR-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
